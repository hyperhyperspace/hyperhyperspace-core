...
describeProxy('[SYN] State sync', () => {
    ...
    if (!haveWebRTC) {
        console.log('[SYN] WebRTC is not available, skipping some sync engine tests.')
    }
...
async function syncInSmallPeerGroup(done: () => void, network: 'wrtc'|'ws'|'mix' = 'wrtc', basePort?: number, useRemoting?: boolean, useSQLite=false, bigElement=false) {
...
async function deepSyncInSmallPeerGroup(done: () => void, network: 'wrtc'|'ws'|'mix' = 'wrtc', basePort?: number, useRemoting?: boolean, useFaultyMessaging=false) {
    ...
    //HistorySynchronizer.opXferLog.setLevel(LogLevel.);
    ...
    for (let i=0; i<size; i++) {
        ...
        if (i===0) {
            ...
            agent.synchronizer.requestLog.setLevel(LogLevel.INFO);
