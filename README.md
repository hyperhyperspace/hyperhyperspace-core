...
> An offline-first shared data library for p2p apps in the browser.
...
Apps that use the HHS model store infromation locally (the current implementation works in a modern web browser, effectively turining it into an autonomus node in a p2p network).
